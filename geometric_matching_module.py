...
class FeatureExtraction(BaseNetwork):  #Extracts features from input data using convolutional layer
...
class FeatureCorrelation(nn.Module): #Computes feature correlation between two sets of features.
    def __init__(self):
...
class FeatureRegression(nn.Module):
    # linear transformation to regress input features to the desired output size.
...
class TpsGridGen(nn.Module):
    def __init__(self, opt, dtype=torch.float):
        ...
        # module for generating the Thin Plate Spline (TPS)
        #  It includes methods to compute the inverse matrix L, apply TPS transformations to points, and perform the forward pass
    ...
    def compute_L_inverse(self,X,Y):
        # Computes the inverse of matrix L for TPS transformation.
    ...
    def apply_transformation(self,theta,points):
        # Applies TPS transformation to points based on the given theta parameters.
    ...
    def forward(self, theta):
        # Forward pass applying TPS transformation on the grid

        warped_grid = self.apply_transformation(theta, torch.cat((self.grid_X, self.grid_Y), 3))
        return warped_grid

class GMM(nn.Module): 
#    GMM aims to align two images by extracting features, computing correlations, and performing regression to generate transformations.
#    generating the grid for geometric transformations using the Thin Plate Spline method.
